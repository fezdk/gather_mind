...
function GatherMindApp() {
  ...
  return <SafeAreaView style={[s.app, { paddingTop: topInset }]} edges={['right', 'left']}>
    ...
    <ThoughtModal visible={thoughtModal} thought={editingThought} thoughts={state.thoughts} appointments={upcomingAppointments(state.appointments)} onClose={() => { setThoughtModal(false); setEditingThoughtId(null); }} onSave={saveThought} onDelete={deleteThought} preselectedId={selectedId ?? ''} />
...
function CloudView({ thoughts, onCapture, onEdit }: { thoughts: Thought[]; onCapture: () => void; onEdit: (thought: Thought) => void }) {
  ...
  return <ScrollView style={s.content} contentContainerStyle={[s.body, { paddingBottom: 112 + bottom }]} keyboardShouldPersistTaps="handled">
    <Text style={s.eyebrow}>See the connections</Text><Text style={s.title}>Mind cloud</Text><Text style={s.subtitle}>Search words or saved themes to gather matching thoughts. Everything stays on this phone.</Text>
    ...
    {!!popularTags.length && <><Text style={s.searchHint}>Saved themes</Text><ScrollView horizontal showsHorizontalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={s.chips}>{popularTags.map((tag) => <Chip key={tag} label={tag} selected={query.trim().toLocaleLowerCase() === tag} onPress={() => setQuery(query.trim().toLocaleLowerCase() === tag ? '' : tag)} />)}</ScrollView></>}
...
function ThoughtModal({ visible, thought, thoughts, appointments, onClose, onSave, onDelete, preselectedId }: { visible: boolean; thought?: Thought; thoughts: Thought[]; appointments: Appointment[]; onClose: () => void; onSave: (input: Pick<Thought, 'text' | 'tags' | 'appointmentId'>, existing?: Thought) => void; onDelete: (thought: Thought) => void; preselectedId: string }) {
  ...
  const tagMatches = suggestedTags(thoughts, completedTags, currentTag);
  function addSuggestedTag(tag: string) { setTags(`${[...new Set([...completedTags, tag])].join(', ')}, `); }
