# Android release runbook — Gather Mind 0.6.5
...
The production profile auto-increments the remote Android version code while the user-visible version remains `0.6.5`. Upload the first AAB manually to Play Console so Google Play App Signing and the application record are established. Later internal-track drafts can be submitted with:
...
The future iOS bundle identifier is also `dk.fez.gathermind`, tablet support is disabled for now, and the configuration declares that the app does not use non-exempt encryption. No iOS build or App Store submission is part of 0.6.5.
